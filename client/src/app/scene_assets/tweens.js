...
export function startTweening() {
    ...
    }, window.l.config.settings.skipintro ? 0 : 250 );
}
...
function flickerEffect() {
    ...
    window.l.current_scene.tweens.doorSignFlickerA = new TWEEN.Tween( dummy )
        ...
        .to( { emissiveIntensity: 0.8 }, window.l.config.settings.skipintro ? 0 : duration * 1000 ) // Start at 0 intensity
        .onUpdate( ( obj ) => {
            updateFlickering( obj );
        } );
    window.l.current_scene.tweens.doorSignFlickerB = new TWEEN.Tween( dummy )
        .delay( duration * 1000 )
        .to( { emissiveIntensity: 0 }, window.l.config.settings.skipintro ? 0 : 0.1 * 1000 )
        .onUpdate( ( obj ) => {
            updateFlickering( obj );
        } );
    window.l.current_scene.tweens.doorSignFlickerC = new TWEEN.Tween( dummy )
        ...
        .easing( TWEEN.Easing.Quadratic.Out )
        .to( { emissiveIntensity: 0.4 }, window.l.config.settings.skipintro ? 0 : 0.2 * 1000 )
        .onUpdate( ( obj ) => {
            updateFlickering( obj );
        } );
    ...
    window.l.current_scene.tweens.doorSignFlickerG = new TWEEN.Tween( dummy )
        .easing( TWEEN.Easing.Quadratic.Out )
        .to( { emissiveIntensity: 1 }, window.l.config.settings.skipintro ? 0 : 0.2 * 1000 )
        .onUpdate( ( obj ) => {
            updateFlickering( obj );
        } )
        .onComplete( () => {
...
function enterTheOffice() {
    ...
    return new TWEEN.Tween( coords, false ) // Create a new tween that modifies 'coords'.
        .to( { x: targetZ }, window.l.config.settings.skipintro ? 0 : 1000 ) // Move to (300, 200) in 1 second.
...
function openDoor( doorRotation ) {
    return new TWEEN.Tween( window.l.current_scene.scene_objects.door.rotation )
        .to( { y: doorRotation }, window.l.config.settings.skipintro ? 0 : 500 ) // Set the duration of the animation
        .onComplete( () => {
...
function dollyUp() {
    return new TWEEN.Tween( window.l.current_scene.camera.position )
        .to( { y: 10.775 }, window.l.config.settings.skipintro ? 0 : 500 ) // Set the duration of the animation
        .onUpdate( () => {
...
function sinkOffice() {
    ...
    return new TWEEN.Tween( coords )
        .to( { y: -50 }, window.l.config.settings.skipintro ? 0 : 10000 ) // Set the duration of the animation
        .onUpdate( () => {
...
function resetCameraPosition( cameraDefaultPosition ) {
    return new TWEEN.Tween( window.l.current_scene.camera.position )
        .to( cameraDefaultPosition, window.l.config.settings.skipintro ? 0 : 1000 )
        .easing( TWEEN.Easing.Quadratic.InOut ) // Use desired easing function
        .onUpdate( () => {
            window.l.current_scene.camera.updateProjectionMatrix();
        } )
...
function resetCameraRotation( cameraDefaultRotation ) {
    return new TWEEN.Tween( window.l.current_scene.camera.rotation )
        .to( cameraDefaultRotation, window.l.config.settings.skipintro ? 0 : 1000 )
        .easing( TWEEN.Easing.Quadratic.InOut ) // Use desired easing function
        .onUpdate( () => {
            window.l.current_scene.camera.updateProjectionMatrix();
        } );
