...
function enterTheOffice() {
    ...
    let targetZ = l.scenograph.actors.player.vehicle.default_camera_distance + l.current_scene.room_depth / 2;
...
function dollyUp() {
    return new TWEEN.Tween( l.scenograph.cameras.player.position )
        ...
        .onUpdate( () => {
            //l.scenograph.cameras.player.lookAt(l.scenograph.actors.player.vehicle.mesh.position);
            l.scenograph.cameras.player.updateProjectionMatrix();
        } )
        .onComplete( () => {
            l.ui.show_menus();
